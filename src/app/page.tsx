          ...
          right={
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <button
                ...
                style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)', padding: '4px 10px', borderRadius: 'var(--radius-btn)', border: '1px solid var(--border-subtle)', background: 'var(--bg-card)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, fontFamily: 'inherit', whiteSpace: 'nowrap' }}
              >
          ...
          {tab === 'pipeline' && (
            <KanbanBoard
              ...
              onClientClick={(clientId) => {
                ...
                if (client) { setTab('clients'); setSelectedClient(client) }
              }}
            />
          )}
          ...
          {tab === 'forecast' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--gap-grid)' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 'var(--gap-grid)' }}>
                <KPICard label="Pipeline total" value={fmtCurrency(kpiPipelineTotal)} sub={`${pipelineDeals.length} deals actifs`} accent />
                <KPICard label="Forecast pondéré" value={fmtCurrency(kpiWeighted)} sub="Montants × % win auto" />
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--gap-grid)' }}>
                <ChartCard title="Win/Loss par type"><WinLossBySegment projects={localProjects} /></ChartCard>
                <ChartCard title="Performance par owner"><OwnerPerformance projects={localProjects} /></ChartCard>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--gap-grid)' }}>
                <ChartCard title="Priorité × Ancienneté" sub="Taille = montant · Couleur = health"><ScatterRisk projects={localProjects} /></ChartCard>
                <ChartCard title="Cohortes par mois de création"><CohortChart projects={localProjects} /></ChartCard>
              </div>
              <ChartCard title="Source d'acquisition"><SourceChart projects={localProjects} /></ChartCard>
            </div>
          )}
              ...
              <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
                  {(['ltv', 'projects'] as const).map(s => (
                    <button key={s} onClick={() => setClientSort(s)} style={{ padding: '5px 12px', fontSize: 'var(--fs-xs)', borderRadius: 'var(--radius-btn)', border: '1px solid var(--border-subtle)', background: clientSort === s ? 'var(--accent-soft)' : 'var(--bg-card)', color: clientSort === s ? 'var(--accent)' : 'var(--text-muted)', cursor: 'pointer', fontFamily: 'inherit' }}>
                      {{ ltv: 'Lifetime Value', projects: 'Projets actifs' }[s]}
                    </button>
                  ))}
                </div>
                <span style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-muted)' }}>{filteredClients.length} client{filteredClients.length !== 1 ? 's' : ''}</span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 14 }}>
                {filteredClients.map(client => (
                  <ClientCard key={client.id} client={client} activeDealsCount={activeDealsMap.get(client.id) || 0} onClick={() => setSelectedClient(client)} />
                ))}
                {filteredClients.length === 0 && (
                  <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: 48, color: 'var(--text-muted)', fontSize: 'var(--fs-sm)' }}>Aucun client trouvé</div>
                )}
              </div>
            </div>
          )}

          {tab === 'settings' && <SalesSettings projects={localProjects} clients={clients} />}
        </main>

        {selectedClient && (
          <ClientDetail client={selectedClient} projects={localProjects} onClose={() => setSelectedClient(null)} />
        )}
      </div>
...
function SalesSettings({ projects, clients }: { projects: Project[]; clients: Client[] }) {
  ...
  const cardStyle: React.CSSProperties = {
    background: 'var(--card-bg)', backdropFilter: 'var(--card-blur)', WebkitBackdropFilter: 'var(--card-blur)',
    border: 'var(--card-border)', borderRadius: 'var(--card-radius)', boxShadow: 'var(--card-shadow)', overflow: 'hidden',
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--gap-grid-lg)' }}>
      <div style={cardStyle}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
          {[
            { title: 'Win % gut feeling', body: 'Valeur saisie manuellement dans Notion (0–100 %). Reflète l\'intuition commerciale. Utilisé par défaut pour pondérer les projections prévisionnelles.' },
            { title: 'Win % auto', body: 'Score calculé par une formule Notion combinant : statut (Scoping +15, Proposal +20, Verbal +30…), budget confirmé (+10), date clôture renseignée (+5), etc. Fallback quand gut feeling = 0.' },
            { title: 'Prévisionnel (forecast)', body: 'Les projets non terminés (hors Won/Lost/Cancelled) sont placés sur leur Expected Close Date. Si absente, ils tombent à currentMonth +3. Le montant est pondéré par le win % sélectionné.' },
            { title: 'Health & Kanban', body: 'Health = formule Notion (❌ Critical, ⚠️ Warning, ✅ OK) basée sur les jours dans l\'étape et les champs manquants. Le Kanban groupe par statut et permet de déplacer un deal d\'une colonne à l\'autre (PATCH Notion).' },
          ].map((item, i) => (
      ...
      <div style={cardStyle}>
        ...
        {filtered.length === 0 ? (
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)', fontSize: 'var(--fs-sm)', fontStyle: 'italic' }}>🎉 Aucun problème dans cette catégorie</div>
        ) : (
                    ...
                    <tr key={idx} style={{ borderTop: '1px solid var(--border-subtle)', background: idx % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
                      <td style={{ padding: '8px 20px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                          ...
                          <span style={{ fontSize: 'var(--fs-2xs)', color: issue.level === 'critical' ? '#ef4444' : issue.level === 'warning' ? '#facc15' : '#60a5fa', fontWeight: 600, textTransform: 'capitalize' }}>{issue.level}</span>
                        </div>
                      </td>
                      <td style={{ padding: '8px 12px' }}><span style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-muted)', textTransform: 'capitalize' }}>{issue.source}</span></td>
                      <td style={{ padding: '8px 12px', color: 'var(--text-primary)', maxWidth: 200 }}><span style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{issue.entity}</span></td>
                      <td style={{ padding: '8px 12px' }}><code style={{ fontSize: 'var(--fs-2xs)', color: 'var(--text-secondary)', background: 'var(--bg-input)', padding: '1px 5px', borderRadius: 3 }}>{issue.field}</code></td>
                      <td style={{ padding: '8px 20px 8px 12px', color: 'var(--text-muted)' }}>{issue.message}</td>
                      <td style={{ padding: '6px 12px' }}>
                        {projectData && (
                          <button onClick={() => setGenericEdit({ entity: 'project', data: projectData })} style={{ background: 'rgba(166,201,206,0.08)', border: '1px solid var(--border-subtle)', borderRadius: 4, color: 'var(--text-muted)', cursor: 'pointer', fontSize: 10, padding: '2px 7px', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>Edit+</button>
                        )}
                      </td>
                    </tr>
                  )
      ...
      {genericEdit && (
        <GenericEditModal entity={genericEdit.entity} data={genericEdit.data} onSave={() => setGenericEdit(null)} onClose={() => setGenericEdit(null)} />
      )}
    </div>
  )
}
